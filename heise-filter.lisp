    ...
    (let ((element (stp:copy (or (first (xpath:all-nodes (xpath:evaluate "/html/body/div/div[@id='content']"
      ...
      (stp:do-recursively (child element)
        (when (typep child 'stp:element)
          (flet ((substitute-attribute-url (attribute-name from to)
                   (alexandria:when-let (url (stp:attribute-value child attribute-name))
                     (setf (stp:attribute-value child attribute-name) (ppcre:regex-replace from url to)))))
            (substitute-attribute-url "href" "^/" "http://www.heise.de/")
            (substitute-attribute-url "src" "^/" "http://www.heise.de/")
            (substitute-attribute-url "src" "^http://www.heise.de/resize/" "http://www.heise.de/"))))
      element)))
